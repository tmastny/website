...
So far we have been taking the weighted averages of subgroup averages to find the total population average. This is usually what we want when we are doing MRP, because regressing gives us the averages for each group. However, many statistics are actually functions of the total statistic. Let's explore poststratifying the total, and see how it relates back to the weighed average.

It doesn't make much sense to estimate the total test scores, but it does for enrollment. I'll use that as our example. 
...
This type of poststratification gives us a pretty decent total estimate.
...
In general, we use poststratification to find the weighted average of subgroup averages to find estimate the population average. And as you'll see in the next post, we want to find these averages by regression instead of the empirical means we showed here.

We also showed that poststratifying averages is a special case of poststratifying the total. With surveys we are typically interested in estimating the mean, but the total statistic gives insight to weighted average correct, and how we could apply this idea to other statistics. Moreover, finding the total can be an useful problem in its own right. There is the classic [German tanks problem](https://en.wikipedia.org/wiki/German_tank_problem), estimating [immigration](http://www.pewresearch.org/fact-tank/2016/09/20/measuring-illegal-immigration-how-pew-research-center-counts-unauthorized-immigrants-in-the-u-s/), or investigating problems [with the Census](http://andrewgelman.com/2010/02/02/problems_with_c_1/). 
