...
How then do we estimate the overall population mean based on `poll_data`? We need to correct based on our Census data, knowing that each mean represents only a certain percentage of the population. This is poststratification. 
...
\end{align}
where $N$ is the total number of subgroups. As we'll see in the next post, we can get the subgroup averages from (multilevel) regressing on the poll outcome for each demographic group. 
...
So far we have been taking the weighted averages of subgroup averages to find the total population average. This is usually what we want when we are doing MRP, because we are usually trying to estimate the total state average. However, averages are actually functions of the total statistic, which can also be useful in its own right. Let's explore poststratifying the total, and see how it relates back to the weighed average.

Estimating total test scores doesn't seem very useful, but estimating total enrollment could be important for policy decisions. I'll use that as our example. 
