...
bind_rows(mod.disag, mod.disag.ps) %>%
  arrange(support) %>%
  filter(state != "") %>%
  ggplot(aes(x = support, y=reorder(state, support), color=model)) + 
  geom_point() + 
...
**** Note: I'm going the wrong way. I need to match each group in the Census with an average score, not just provide Census info for each average. 

**** Sum each `cpercent.state` to see if you get 1 to demonstrate the impact of missing subgroups

In this case, poststratification is producing some really weird results, claiming that half a dozen states have zero support for gay marriage. Let's dive into South Dakota and see what's going on:
...
grp.means %>%
  filter(state == 'CA') %>%
  select(-region, -state, -support)
```
We only have a state percent for 2 groups, both with zero support.
```{r}
Census %>%
  filter(state == 'CA') %>%
  select(age.cat, edu.cat, kerry.04,
         race.female, age.edu.cat, p.relig, cpercent.state)
```
```{r}
marriage.data %>%
  filter(state == 'CA') %>%
  select(poll, age.cat, edu.cat, kerry.04,
         race.female, age.edu.cat, p.relig, yes.of.all)
```
