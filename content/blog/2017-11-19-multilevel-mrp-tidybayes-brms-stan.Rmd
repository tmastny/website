...
The `lme4` package provides a method `predict` to easily make predictions on new data. This is a generic implementation of rebuilding the link function based off of regression coefficients. ^[Here, the Census by design provides every combination of demographic groups within each state. If that wasn't the case, I would use [`modelr](https://github.com/tidyverse/modelr)::data_grid` to generate all combninations of a given data set.]

Moreover, we can ues this opportunity to directly specify the new data. Unlike the empirical averages, our model can estimate the level of opinion for any subgroup within any state. So we want to predict based on data that has every subgroup combination for every state. The Census data already provides this, but here I want to highlight a more general tool we can use to generate all combinations of data. We can do this quickly with the `modelr` package:

```{r}
state_preds <- predict(approx.mod, newdata=Census, allow.new.levels=TRUE,
                       type='response')
```

Since these new values were estimated from the Census data, we now have decent estimates for Alaska and Hawaii even though the survey had no respondents from those states. To build those estimations, the model used the population level state intercept, but was able to use the demographic information within the states to estimate the level of support of all the groups within Alaska and Hawaii, which will be necessary for poststratification. 


Using the [`broom`](https://github.com/tidyverse/broom) package, we can extract the regression coefficients into a tidy dataframe
