...
The aim of the MRP Primer is to estimate state level opinions for gay marriage based on a potentionally non-representative survey data. That requires poststratification. As was done in the last post, we are going to use external Census data to scale the average support of each demographic group in proportion to its percentage of the state population.

Multilevel regression is how we are going to estimate the average support for each group. As we'll see in this post, and in many [examples](http://andrewgelman.com/2014/01/21/everything-need-know-bayesian-statistics-learned-eight-schools/) in the literature, multilevel models allow us to partially pool information across similar groups, as well as add regression predictor. These model estimates turn out to be much better than empirical averages. 
...
Now the data is broken down into the demographic groups. The next step is to find the percentage of the total state population each group is for poststratification.
