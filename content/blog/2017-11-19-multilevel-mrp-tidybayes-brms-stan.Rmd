...
The left plot shows a lot of variation between the poststratified averages. The right plot indicates that every poststratified state average is pushed near zero. That's not necessarily a problem in its down right, but we should still debug the model.

Within each state, each group is a percentage of the total. And the percentages of each group should add up to one, the entire state population:
...
What's going own in our model? Let's sum of the percent of demographic groups we have in each state.
...
So clearly the survey does not have samples from each demographic group within each state. 
