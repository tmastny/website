...
So what does this full Bayesian model give us that the approximate model lacks? The most obvious is a total accounting of the uncertainty inherent in the model. For example, let's take a look at the standard deviation of group-level intercepts:^[Technically, we could calculate the standard error from the approximate model. However, it may not [be accurate](https://stackoverflow.com/questions/31694812/standard-error-of-variance-component-from-the-output-of-lmer) and it is computationally expensive. It took way longer to calculate the standard error via bootstrapping compared to just fitting the entire model in `brms`.]
