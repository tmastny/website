...
```{r}
Census <- Census %>% rename(state = cstate)
...
Census <- Census %>%
  ...
  mutate(age.edu.cat = (cage.cat - 1) + cedu.cat) %>%
  mutate(p.relig = p.evang + p.mormon)
```
...
This simplicity comes at a cost. As demonstrated in the previous post, this empirical mean is not representative of the actual state mean if the respondents within are not in proportion to each group's percentage of the total population. So let's build the poststratified disaggregated model.
